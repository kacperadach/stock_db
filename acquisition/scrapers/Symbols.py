...
class SymbolScraper(StockDbBase):
    ...
    def get_next_input(self):
        ...
        if current_region not in self.query_dict.keys():
            symbol_request = YahooFinanceSymbolRequest(regions=[current_region], offset=0)
        ...
        if offset <= self.query_dict[current_region]['total']:
            ...
            symbol_request = YahooFinanceSymbolRequest(regions=[current_region], offset=new_offset)
