...
class MarketWatchScraper(StockDbBase):
    ...
    def get_symbols(self):
        self.symbols = list(self.db.find(SYMBOL_COLLECTION, {}, {'symbol': 1, 'instrument_type': 1, 'Exchange': 1, 'Country': 1, 'countryCode': 1}))
    ...
    def process_data(self, queue_item):
        ...
        data['instrument_type'] = metadata['symbol']['instrument_type']
        ...
        del data['data']
        collection_name = self.get_collection_name(data['instrument_type'])
        documents = []
        for day, d in days.iteritems():
            ...
            new_document['exchange'] = metadata['Exchange']
            documents.append(new_document)

        trading_days = map(lambda x: x['trading_date'], documents)
        existing_documents = list(self.db.find(collection_name, {'trading_date': {'$in': trading_days}, 'symbol': metadata['symbol']['symbol'], 'instrument_type': data['instrument_type']}, {'trading_date': 1, 'data': 1}))
